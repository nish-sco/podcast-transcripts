...
1. **Get the files.** If the current folder is empty, clone the repo straight into it (`git clone https://github.com/nish-sco/podcast-transcripts.git .`), so this folder becomes the project folder. If the folder isn't empty, clone into a `podcast-transcripts` subfolder and tell the user to select that subfolder in future sessions. If `git` isn't available, download https://github.com/nish-sco/podcast-transcripts/archive/refs/heads/main.zip. It unzips into a `podcast-transcripts-main` subfolder, so move that subfolder's contents (including the hidden `.claude` folder and `.env.example`) up into the current folder. Whichever route you take, finish with `README.md` sitting directly in the folder the user selected, or, if you had to use a subfolder, tell the user exactly which folder to select from now on. `.env` must go in that same folder.
